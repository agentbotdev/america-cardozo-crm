...
const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, onSave, taskToEdit }) => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const { data } = await supabase.from('propiedades').select('id, titulo').ilike('titulo', `%${propSearch}%`).limit(5);
      setPropResults(data || []);
    }, 300);
    return () => clearTimeout(t);
  }, [propSearch]);
                ...
                {propResults.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-100 rounded-2xl shadow-xl z-10 overflow-hidden">
                    {propResults.map((p: any) => (
                      <button key={p.id} type="button" onClick={() => { setForm({ ...form, propiedad_id: p.id }); setSelectedPropName(p.titulo); setPropSearch(''); setPropResults([]); }}
